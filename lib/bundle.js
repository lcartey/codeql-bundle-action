...
class Bundle {
    constructor(octokit, tag, bundlePath, tmpDir) {
        this.octokit = octokit;
        this.tag = tag;
        this.bundlePath = bundlePath;
        this.tmpDir = tmpDir || process.env.RUNNER_TEMP || "/tmp";
    }
    static async getBundleByTag(token, tag) {
        const octokit = new rest_1.Octokit({ baseUrl: "https://api.github.com" });
        ...
        const asset = bundleRelease.assets.find((asset) => asset.name === 'codeql-bundle.tar.gz');
        if (asset) {
    ...
    static async getLatestBundle(token) {
